...
class CAN():
    def __init__(self, n, seed=10, mode='run', random_pointers=None):
        self.n = n
        self.seed = seed
        self.nodes = []
        self.importance = []
        ...
        self.mode = mode
        self.random_pointers = random_pointers

    def add_node(self, node_id):

        def split(rect):
            if rect[2] - rect[0] == rect[3] - rect[1]:
                return (rect[0], rect[1], rect[0] + (rect[2] - rect[0]) / 2, rect[3]), (rect[0] + (rect[2] - rect[0]) / 2, rect[1], rect[2], rect[3])
            else:
                return (rect[0], rect[1], rect[2], rect[1] + (rect[3] - rect[1]) / 2), (rect[0], rect[1] + (rect[3] - rect[1]) / 2, rect[2], rect[3])

        def check_neighbor(rect1, rect2):

            def check_first(rect1, rect2):
                x1, y1, x2, y2 = rect1
                x12 = (x1 + x2) / 2
                y12 = (y1 + y2) / 2
                n_zeros = False
                n_zeros = n_zeros or (compute_distance((x1, y1), rect2) == 0 and compute_distance((x1, y2), rect2) == 0 and compute_distance((x1, y12), rect2) == 0)
                n_zeros = n_zeros or (compute_distance((x1, y2), rect2) == 0 and compute_distance((x2, y2), rect2) == 0 and compute_distance((x12, y2), rect2) == 0)
                n_zeros = n_zeros or (compute_distance((x2, y1), rect2) == 0 and compute_distance((x1, y1), rect2) == 0 and compute_distance((x12, y1), rect2) == 0)
                n_zeros = n_zeros or (compute_distance((x2, y2), rect2) == 0 and compute_distance((x2, y1), rect2) == 0 and compute_distance((x2, y12), rect2) == 0)
                return n_zeros and rect1 != rect2

            return check_first(rect1, rect2) or check_first(rect2, rect1)

        rect1, rect2 = split(self.nodes[node_id].rect)
        ...
        for key in list(table.keys()):
            if check_neighbor(rect2, table[key].rect):
                self.nodes[-1].add_neighbor(table[key])
                table[key].add_neighbor(self.nodes[-1])
            if not check_neighbor(rect1, table[key].rect):
    ...
    def add_random_pointers(self):
        if self.random_pointers is not None:
            for i in range(self.random_pointers):
                ind1, ind2 = 1, 1
                while ind1 == ind2:
